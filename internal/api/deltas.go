...
func handleAcceptDelta(deps AppDeps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := deps.Profile.ApplyDelta(profileDelta); err != nil {
			// Delta is already marked accepted but apply failed. Log critically
			// so this can be diagnosed; the delta record shows what should have
			// been applied and can be retried manually.
			slog.Error("delta accepted but apply failed — requires manual recovery",
				"delta_id", id, "error", err)
