...
// UnreviewDelta resets a reviewed delta back to unreviewed (accepted = NULL,
// reviewed_at = NULL). Used to roll back acceptance when ApplyDelta fails.
// Returns ErrNotFound if the delta does not exist.
func (s *Store) UnreviewDelta(id string) error {
	res, err := s.db.Exec(`
		UPDATE pending_profile_deltas SET accepted = NULL, reviewed_at = NULL
		WHERE id = ?`, id,
	)
	if err != nil {
		return err
